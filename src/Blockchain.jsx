...
// For manipulating with Global States => react-hooks-global-state
...
const connectWallet = async () => {
  try {
    ...
    const accounts = await ethereum.request({ method: "eth_requestAccounts" }); // requesting Metamask accounts
    setGlobalState("connectedAccount", accounts[0].toLowerCase()); // setting GlobalState
  } catch (error) {
    reportError(error);
  }
};
...
const isWalletConnected = async () => {
  try {
    ...
    const accounts = await ethereum.request({ method: "eth_accounts" }); // getting accounts
...
const performContribute = async (amount) => {
  try {
    ...
    //* Will aiways refer to the DAO SC
...
const listVoters = async (proposerId) => {
  try {
    const contract = await getEthereumContract();
    const votes = await contract.methods.getVotesOf(proposerId).call();
    return votes;
  } catch (error) {
    reportError(error);
  }
...
// does here we can handle all types of Error ??
